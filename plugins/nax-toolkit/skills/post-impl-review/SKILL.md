...
3. **Dispatch BOTH workers IN PARALLEL** — one message, two Agent/Task calls. Each reviews the **whole diff**; never fan out per-package (Integration needs cross-package boundaries holistically). Neither worker reads this Step 0 or the 20 KB `SKILL.md` — each reads only the slim `references/worker-protocol.md` (shared mechanics: diff commands, noise filter, collaborators, severity table, output format) plus its own dimension file, so a worker's context is the diff itself, not this skill's prose. Give each a self-contained prompt:
