...
| Backgrounding the review worker, then scheduling a wakeup / polling to "wait" for it | The review runs synchronously — the Skill call blocks and returns the findings inline. Never background it or call a wait/wakeup tool (that path errors, e.g. a wakeup with no prompt); just await the result (Step 4) |
