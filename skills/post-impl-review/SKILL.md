...
3. **Decide the fan-out from diff size** — you already have the `--stat` summary from step 2, so this is free. Measure the diff **after** the Step 3 noise filter (exclude lockfiles/generated/binary):
...
4. **Merge and print** (Step 6): With the **single combined worker**, there is nothing to merge — take its findings as-is and skip the cross-worker dedupe. With **two workers**, combine both findings sets and dedupe the cross-worker overlap. Either way, sort and print the header + unified findings + single verdict, and relay it verbatim to the caller. If a worker fails outright, note it in the header and continue with whatever findings you have rather than aborting the whole review.
