...
| **post-impl-review** | Post-implementation review of changed code against **the one feature spec that drove it** — named by the caller, or belonging to the nax run just completed. Diffs against the repo's default branch and reviews across five dimensions — compliance, drift, integration, convention compliance (project rule files), and code quality — printing severity-graded findings (CRITICAL/HIGH/MEDIUM/LOW) with a unified verdict. Requires that spec: it never guesses one by searching, and stops rather than reviewing against a spec that did not drive the change. For direct implementation with no spec behind it, run `--phase quality` for the spec-independent pass. |
...
- **Plugin installs but no skills appear** — the installed payload must contain a
  real `skills/` directory inside `plugins/<plugin-name>/`. A symlink that points
